...
public class SalesOrderService : GeneralService<SalesOrderHeader>, ISalesOrderService
{
    ...
    public SaveResult Insert(SalesOrderRequest data)
    {
        ...
        try
        {
            ...
            if (((checkQty && (!data.IsSoDlv || !data.IsSoInv)) || data.IsSoDlv || data.IsSoInv) && IsQtyExcess(data.WarehouseCode, data.ItemDetails, null, (data.IsSoDlv || data.IsSoInv)))
            {
            ...
            foreach (var item in data.ItemDetails)
            {
                ...
                if (data.IncludeTax)
                {
                    item.TaxAmount = (item.UnitPrice - item.Disc) - ((item.UnitPrice - item.Disc) / (1 + (taxData.Rate / 100)));
                    item.ExemptTaxAmount = (item.UnitPrice - item.Disc) - ((item.UnitPrice - item.Disc) / (1 + (taxData.ExemptRate / 100)));
                ...
                else
                {
                    ...
                    item.ExemptTaxAmount = (item.UnitPrice - item.Disc) * (taxData.ExemptRate / 100);
                ...
                if (bonusPromoMulti.Any() && orderDetail.LineNo == data.ItemDetails.Count())
                {
    ...
    public SaveResult Update(SalesOrderRequest data)
    {
        ...
        try
        {
            ...
            if ((checkQty && (!data.IsSoDlv || !data.IsSoInv)) || (data.IsSoDlv || data.IsSoInv) && IsQtyExcess(data.WarehouseCode, data.ItemDetails, data.Code, (data.IsSoDlv || data.IsSoInv)))
            {
                result.Message = "Data order penjualan tidak bisa disimpan karena qty barang yang dipesan lebih besar dari qty " + ((data.IsSoInv || data.IsSoDlv) ? "sistem." : "tersedia.");
                return result;
            }
    ...
    private bool IsQtyExcess(string warehouseCode, IEnumerable<SalesOrderDetail> items, string code, bool isSaveDO)
    {
