...
public class SalesReturnReportService : ISalesReturnReportService
{
    ...
    public DataSourceResult GetData(int type, string startDate, string endDate, int? salesId, string custCode, string status, int? itemId, string code, bool isDetail, int? unitId, int? categoryId)
    {
        ...
        var srDetailData = _db.ReportByDetailSRs.FromSqlRaw(@"SELECT sr.[Date], sr.Code,
                            ...
                            sr_d.TaxAmount * sr_d.Qty AS TotalTaxAmount, sr_d.ExemptTaxAmount * sr_d.Qty AS TotalExemptTaxAmount, sr_d.NettPrice * sr_d.Qty AS TotalNettPrice,
                            CASE sr.Mark
                                WHEN 'A' THEN 'Aktif'
                                WHEN 'V' THEN 'Void'
                                WHEN 'PS' THEN 'Dikirim Sebagian'
        ...
        if (!isDetail)
        {
            if (type == 1)
            {
                ...
                srData.Add(new Entity.Sales.ReportBySR
                {
                    ...
                    ExemptTaxAmount = srData.Sum(x => x.ExemptTaxAmount),
            ...
            else if (type == 2)
            {
                ...
                foreach (var itemCust in custData)
                {
                    ...
                    itemCust.ExemptTaxAmount = srDetailData.Where(x => x.CustCode == itemCust.Code).Sum(x => x.TotalExemptTaxAmount);
            ...
            else if (type == 3)
            {
                ...
                foreach (var item in itemData)
                {
                    ...
                    item.ExemptTaxAmount = srDetailData.Where(x => x.ItemInitial == item.Initial && x.UnitId == item.UnitId).Sum(x => x.TotalExemptTaxAmount);
            ...
            else if (type == 4)
            {
                ...
                foreach (var item in itemCategoryData)
                {
                    ...
                    item.ExemptTaxAmount = srDetailData.Where(x => x.CategoryId == item.CategoryId && x.UnitId == item.UnitId).Sum(x => x.TotalExemptTaxAmount);
            ...
            else
            {
                ...
                srDetailData.Add(new Entity.Sales.ReportByDetailSR
                {
                    ...
                    TotalExemptTaxAmount = srDetailData.Sum(x => x.TotalExemptTaxAmount),
