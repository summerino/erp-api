        ...
        foreach (var itemInv in invData)
        {
            if (!string.IsNullOrEmpty(startDate))
            {
                if (itemInv.Date < Convert.ToDateTime(startDate))
                {
                    ...
                    var totCdm = invDMData.Where(x => x.InvCode == itemInv.Code && dmData.Where(y => y.Date >= Convert.ToDateTime(startDate)).Select(y => y.Code).Contains(x.DebitMemoCode)).Sum(x => x.DebitMemoAmount);
