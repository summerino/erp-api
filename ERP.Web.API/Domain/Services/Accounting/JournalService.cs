...
namespace ERP.Web.API.Domain.Services.Accounting
{
    public class JournalService : IJournalService
    {
        ...
        private IEnumerable<Journal> ProcessSaleJournal(DateTime dateTime, List<SystemParameter> systemParam, List<Item> items, List<Tax> taxes)
        {
            ...
            foreach (var itemData in DlvData)
            {
                ...
                foreach (var itemDetail in DlvDetailData)
                {
                    ...
                    CalculateHPP(nonVoidSM, smData.WarehouseCode, smData.ItemId, smData.RefDetailId1, "DO");
            ...
            foreach (var itemData in RtnData)
            {
                if (itemData.RtnHeader.Type != 1)
                {
                    ...
                    foreach (var itemDetail in RtnDetailData)
                    {
                        ...
                        if (smData == null) continue;
                        var nonVoidSM = RemoveVoidSM(_db.StockMutations.ToList());
                        CalculateHPP(nonVoidSM, smData.WarehouseCode, smData.ItemId, smData.RefDetailId1, "PR");
                    ...
                    journals.Add(new Journal
                    {
                        ...
                        Period = itemData.RtnHeader.Date.ToString("yyyyMMdd"),
                        Type = "D",
                        Amount = journals.Where(x => x.Code == itemData.RtnHeader.Code && new[] { 2, 3 }.Contains(x.Group)).Sum(s => s.Amount),
                        SrcTrans = "PR"
                    });
                        ...
                        foreach (var itemRcvData in RcvData)
                        {
                            ...
                            foreach (var itemDetail in RcvDetailData)
                            {
                                ...
                                journals.Add(new Journal
                                {
                                    ...
                                    Type = "D",
                                    Amount = itemData.RtnHeader.Type == 2 ? hppData.ContainsKey(itemDetail.RcvDetail.ItemId) ? hppData[itemDetail.RcvDetail.ItemId] : 0 : (itemDetail.RcvDetail.NettPrice - itemDetail.RcvDetail.TaxAmount) * itemDetail.RcvDetail.Qty,
                                    SrcTrans = "RCV"
                                });
                                ...
                                journals.Add(new Journal
                                {
                                    ...
                                    Period = itemRcvData.RcvHeader.Date.ToString("yyyyMMdd"),
                                    Type = "D",
                                    Amount = itemData.RtnHeader.Type == 2 ? 0 : hppData.ContainsKey(itemDetail.RcvDetail.ItemId) ? hppData[itemDetail.RcvDetail.ItemId] > 0 ? hppData[itemDetail.RcvDetail.ItemId] : 0 : 0,
                                    SrcTrans = "RCV"
                                });
                            }
                            //Hutang - AP
                            journals.Add(new Journal
                            {
                                ...
                                Period = itemRcvData.RcvHeader.Date.ToString("yyyyMMdd"),
                                Type = "C",
                                Amount = itemData.RtnHeader.Type == 2 ? journals.First(x => x.Code == itemData.RtnHeader.Code && x.Group == 1).Amount : itemRcvData.RcvHeader.Total,
                                SrcTrans = "RCV"
                            });
                ...
                else
                {
                    ...
                    var ciValue = journals.FirstOrDefault(x => x.Code == itemData.RtnHeader.Code && x.Group == 1).Amount - journals.Where(x => x.Code == itemData.RtnHeader.Code && new[] { 2, 3, 4 }.Contains(x.Group)).Sum(x => x.Amount);
                    journals.Add(new Journal
                    {
                        Code = itemData.RtnHeader.Code,
                        LineNo = 1,
                        ...
                        TypeCode = "PR",
                        Notes = ($"{systemParam.FirstOrDefault(x => x.Code == (ciValue < 0 ? "JR_PREFIX_OTH_EXPENSE" : "JR_PREFIX_OTH_INCOME"))?.Value ?? ""} {itemData.Supplier.Initial}").Trim(),
                        RefCode1 = "",
                        Group = 5,
                        CurrCode = itemData.RtnHeader.CurrCode,
                        Period = itemData.RtnHeader.Date.ToString("yyyyMMdd"),
            ...
            foreach (var itemData in RtnData)
            {
                if (itemData.RtnHeader.Type != 1)
                {
                    ...
                    foreach (var itemDetail in RtnDetailData)
                    {
                        ...
                        if (smData == null) continue;
                        var nonVoidSM = RemoveVoidSM(_db.StockMutations.ToList());
                        CalculateHPP(nonVoidSM, smData.WarehouseCode, smData.ItemId, smData.RefDetailId1, "SR");
                        smData = _db.StockMutations.FirstOrDefault(x => x.RefDetailId1 == itemDetail.RtnDetail.Id && x.RefCode1 == itemDetail.RtnDetail.Code);
                        var resultHpp = smData.BaseNettPrice > 0 ? smData.BaseNettPrice * smData.BaseQty : 0m;
                    ...
                    journals.Add(new Journal
                    {
                        ...
                        Period = itemData.RtnHeader.Date.ToString("yyyyMMdd"),
                        Type = "C",
                        Amount = Math.Abs(itemData.RtnHeader.Total) - journals.Where(x => x.Code == itemData.RtnHeader.Code && x.Group == 1).Sum(x => x.Amount),
                        SrcTrans = "SR"
                    });
                    ...
                    journals.Add(new Journal
                    {
                        ...
                        Period = itemData.RtnHeader.Date.ToString("yyyyMMdd"),
                        Type = "D",
                        Amount = Math.Abs(itemData.RtnHeader.Total) - journals.Where(x => x.Code == itemData.RtnHeader.Code && new[] { 2, 3 }.Contains(x.Group)).Sum(x => x.Amount),
                        SrcTrans = "SR"
                    });
                    ...
                    if (DlvData.Any())
                    {
                        foreach (var itemDlvData in DlvData)
                        {
                            ...
                            journals.Add(new Journal
                            {
                                ...
                                Period = itemDlvData.DlvHeader.Date.ToString("yyyyMMdd"),
                                Type = "D",
                                Amount = journals.FirstOrDefault(x => x.Code == itemData.RtnHeader.Code && x.Group == 4).Amount,
                                SrcTrans = "DLV"
                            });
        ...
        private IEnumerable<Journal> ProcessAdjustmentJournal(DateTime dateTime, List<SystemParameter> systemParam)
        {
            ...
            foreach (var itemData in adjData)
            {
                ...
                foreach (var itemDetail in adjDetailData)
                {
                    ...
                    var nonVoidSM = RemoveVoidSM(_db.StockMutations.ToList());
                    CalculateHPP(nonVoidSM, smData.WarehouseCode, smData.ItemId, smData.RefDetailId1, "ADJ");
            ...
            foreach (var itemData in dataHeader)
            {
                ...
                foreach (var itemDetail in dataDetail)
                {
                    ...
                    if (itemData.Type != "IN")
                    {
                        ...
                        CalculateHPP(nonVoidSM, smData.WarehouseCode, smData.ItemId, smData.RefDetailId1, srcType);
        ...
        private void CalculateHPP(IEnumerable<StockMutation> stockMutations, string whCode, int itemId, long id, string srcCode)
        {
            decimal latestQty = 0;
            decimal latestStockValue = 0;
            decimal hpp = 0;

            var firstSM = stockMutations.OrderBy(y => y.Date).FirstOrDefault(x => x.WarehouseCode == whCode && x.ItemId == itemId && x.Src == "BB");
            if (firstSM == null)
                firstSM = stockMutations.OrderBy(y => y.Date).FirstOrDefault(x => x.WarehouseCode == whCode && x.ItemId == itemId && x.Src == "RCV");
            ...
            if ((firstSM?.BaseNettPrice ?? 0m) != 0m)
            {
                ...
                var listSM = stockMutations.Where(x => x.Id != firstSM.Id 
                            ...
                            && x.WarehouseCode == whCode && x.ItemId == itemId 
                            && x.Date >= firstSM.Date 
