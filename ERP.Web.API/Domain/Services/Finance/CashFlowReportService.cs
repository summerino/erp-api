...
public class CashFlowReportService : ICashFlowReportService
{
        ...
        if (type == 1)
        {
            sql += $@"
                ...
                FROM (
                    ...
                    ,'' AS CbCode,'' AS TransCode,'' AS Notes
                    ,SUM(AmountOc) AS AmountOc,SUM(AmountIdr) AS AmountIdr
					FROM #TmpCbResult
    ...
    private static string GetSource(string coaCode, string dateFrom, string dateTo)
    {
        ...
        string sql = $@"
                ...
                ,cte_cb_d_src AS (
                    SELECT
                    CbCoa,CoaCode,CoaName,TypeTranIO,[Type]
                    ,Code,TransCode,Notes
                    ,AmountOc,AmountOc * RateEb AS AmountIdr
                    FROM cte_cb_all_src
                ...
                ,cte_cb_icb_src AS (
                    ...
                    GROUP BY CbCoa,CoaCode,CoaName,Code,TransCode,Notes
                ...
                ,cte_cb_beg_src AS (
                    ...
                    ,'' AS CbCode,'' AS TransCode,'' AS Notes
                    ,ISNULL(SUM(AmountOc),0) AS AmountOc,ISNULL(SUM(AmountOc * RateBb),0) AS AmountIdr
                    FROM cte_cb_all_src
                    ...
                    --,'' AS CbCode,'' AS TransCode,'' AS Notes
                    --,0.00 AS AmountOc,ISNULL(SUM(AmountOc * (RateEb - RateBb)), 0) AS AmountIdr
                    --FROM cte_cb_all_src
                    --WHERE 1 = 1 {whDateFrom2}
                    --GROUP BY CbCoa
