...
public class TransferStockController : ControllerBase
{
    ...
    private const int MenuId = (int)Menu.TransferStock;
    ...
    public IActionResult OnDelete(string code, TransferStockRequest data)
    {
        ...
        var result = _ts.Delete(data.Code, _claim.UserId);
        return Ok(result);
    }
    ...
    private (bool, string) Validate(TransferStockRequest data, bool onDelete = false, bool checkSeenByOther = true)
    {
        ...
        if (checkSeenByOther && !_activeTrans.SeenByOthers("TS", data.Code, _claim.UserId))
        {
