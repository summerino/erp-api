...
namespace ERP.Web.API.Controllers.Sales
{
    ...
    public class SalesReturnController : ControllerBase
    {
        ...
        private const int MenuId = (int)Menu.SalesReturn;

        public SalesReturnController(ISalesReturnService rtn, IUnitOfMeasurementService uom,
        ...
        [HttpDelete("{code}")]
        public IActionResult OnDelete(string code, SalesReturnRequest data)
        {
            // Checking role authorization
            ...
            var result = _rtn.Delete(data.Code, _claim.UserId);

            return Ok(result);
        }

        private (bool, string) Validate(SalesReturnRequest data, bool onDelete = false)
        {
