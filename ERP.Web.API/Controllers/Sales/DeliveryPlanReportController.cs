using ERP.Web.API.Domain.Interfaces.Sales;
using ERP.Web.API.Model;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using System.Linq.Dynamic.Core;

namespace ERP.Web.API.Controllers.Sales
{
    [Route("delivery-plan-report")]
    [ApiController]
    public class DeliveryPlanReportController : ControllerBase
    {
        private readonly IDeliveryPlanReportService _dpr;
        public DeliveryPlanReportController(IDeliveryPlanReportService dpr)
        {
            _dpr = dpr;
        }

        [HttpGet]
        public IActionResult GetData(int type, string startDate, string endDate,
        string custCode, string status, int? itemId,
        string code, bool isDetail, int? unitId,
        int? categoryId)
        {
            var result =
                _dpr.GetData(type, startDate, endDate,
                    custCode, status, itemId,
                    code, isDetail, unitId,
                    categoryId);

            return Ok(new ApiResponse
            {
                RowCount = result.Total,
                TableData = result.Data.ToDynamicList()
            });
        }
    }
}
