...
{
    public partial class AddViewVwApAndVwAr : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sql = @"ALTER VIEW [Finance].[vwPR] AS
SELECT B.Code, B.SupCode, S.[Name] AS SupName, [Date],CurrCode, Rate, Amount, Used, Amount - Used as Remaining, B.Notes
...
WHERE P.Used < P.Amount AND (P.Mark = 'A' OR P.Mark = 'PU')";
            ...
            sql = @"ALTER VIEW [Finance].[vwSR] AS
SELECT B.Code, B.CustCode, C.[Name] AS CustName, [Date],CurrCode,Rate , Amount, Used, Amount - Used as Remaining, B.Notes
