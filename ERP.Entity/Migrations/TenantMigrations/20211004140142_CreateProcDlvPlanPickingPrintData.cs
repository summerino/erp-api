...
{
    public partial class CreateProcDlvPlanPickingPrintData : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Create procedure dbo.sp_get_dlv_plan_picking_print_data
            var sql = @"CREATE PROCEDURE [dbo].[sp_get_dlv_plan_picking_print_data]
	@code varchar(17),
	...
	BEGIN
		WITH cte_dlv_plan_src AS (
			SELECT dlv_plan_h.Code, dlv_plan_h.[Date], dlv_plan_h.VehicleId, dlv_plan_h.DriverId, dlv_plan_h.WarehouseCode,
				...
				CASE WHEN ISNULL(si_d_1.DOCode, '') = '' THEN dlv_plan_d.TransCode
					ELSE si_d_1.DOCode END AS DOCode
			FROM (
				SELECT *
			...
			LEFT JOIN Sales.SalesInvoiceHeader si_h_1
				ON si_h_1.Code = dlv_plan_d.TransCode
			LEFT JOIN Sales.SalesInvoiceDetail si_d_1
				ON si_d_1.Code = si_h_1.Code
		)
		,cte_normal_src AS (
			SELECT cte.*,
...
END";
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            var sql = @"DROP PROCEDURE [dbo].[sp_get_dlv_plan_picking_print_data]";
            migrationBuilder.Sql(sql);
        }
    }
}
