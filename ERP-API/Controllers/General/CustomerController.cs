...
namespace ERP_API.Controllers.General
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        private const int _menuId = (int)Menu.Customer;

        public CustomerController(ICustomerService customer, IClaimService claim, IAuthService auth)
        {
            _customer = customer;
            _claim = claim;
            _auth = auth;
        ...
        public IActionResult GetList(string filters, string sorts) 
        {
            var data =
                _customer.GetLists(
                    ...
                    .Select(x => new
                    {
                        x.Code, x.Initial, x.Name, x.Address1, x.Phone, x.Fax, x.PaymentTermId
                    })
                    .ToList<dynamic>();
