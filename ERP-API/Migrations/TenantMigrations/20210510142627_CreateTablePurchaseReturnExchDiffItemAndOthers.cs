...
{
    public partial class CreateTablePurchaseReturnExchDiffItemAndOthers : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Set default value for Notes column in table Purchasing.PurchaseReturnHeader
            var sql = @"UPDATE Purchasing.PurchaseReturnHeader SET Notes = '' WHERE Notes IS NULL";
...
BEGIN TRY
	...
	INSERT INTO Inventory.StockMutation
		SELECT @warehouseCode, @date, ItemId, UomId, UnitId, Qty, 0, 0, Code, Id,
			CASE WHEN @dlvCode IS NOT NULL THEN @dlvCode ELSE NULL END,
			'SR'
		FROM #tmp_sr sr
		WHERE NOT EXISTS (
			SELECT Id
...
END CATCH";
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
			...
			var sql = @"DROP PROCEDURE [dbo].[sp_update_stock_mutation_from_sr]";
			migrationBuilder.Sql(sql);
		}
    }
}
